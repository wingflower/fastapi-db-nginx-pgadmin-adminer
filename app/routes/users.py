...
from app.models.user_models import MessageOk
from app.models.api_key_models import (
    AddApiKey, GetApiKeys, GetApiKeyList,
    GetAPIWhiteLists, CreateAPIWhiteLists
)
...
async def create_api_keys(request: Request, params: AddApiKey, session: Session = Depends(db.session)):
    """
    API KEY 생성
    :param request:
    :param key_info:
...
async def update_api_keys(request: Request, key_id: int, key_info: AddApiKey):
    """
...
async def create_api_keys(request: Request, key_id: int, ip: CreateAPIWhiteLists, session: Session = Depends(db.session)):
    user = request.state.user
    await check_api_owner(user.id, key_id)
    import ipaddress
    try:
