...
class MaxKeyCountEx(APIException):
    def __init__(self, ex: Exception = None):
        super().__init__(
            status_code=StatusCode.HTTP_400,
            msg=f"API 키 생성은 {Config.data['MAX_API_KEY']}개 까지 가능합니다.",
...
class MaxWLCountEx(APIException):
    def __init__(self, ex: Exception = None):
        super().__init__(
            status_code=StatusCode.HTTP_400,
            msg=f"화이트리스트 생성은 {Config.data['MAX_API_WHITELIST']}개 까지 가능합니다.",
...
class NoRunningUrlEx(APIException):
    def __init__(self, url: str, ex: Exception = None):
        super().__init__(
            status_code=StatusCode.HTTP_444,
            msg=f"해당 URL의 응답이 없습니다.",
            detail=f"No running url : {url}",
            code=f"{StatusCode.HTTP_444}{'10'.zfill(4)}",
            ex=ex,
        )
